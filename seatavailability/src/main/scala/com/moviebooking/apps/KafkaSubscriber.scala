...
import com.moviebooking.aggregates.{Event, Initialized, SeatAvailability, SeatsReserved}
...
object KafkaSubscriber extends App with JsonSupport {
  ...
  private val consumer: KafkaConsumer[String, String] = consumerSettings.createKafkaConsumer()
  val partition = 0
  val fromOffset = 0L
  ...
  def readJson(record: ConsumerRecord[String, String]):Future[Done] = Future {
    ...
    event match {
      case init:Initialized ⇒
        ...
        redisConnection.sync().set(init.id, Json.toJson(SeatAvailability(init.seats)).toString())
      case reserved:SeatsReserved⇒
        ...
        redisConnection.sync().set(reserved.id, Json.toJson(availability).toString())
    }
    Done
  }
}
