...
class ClusterSettings(listenPort: Int = 0) {
  ...
  def seedNodes: List[String] = {
    val seeds = List(s"${hostname}:${ClusterSettings.seedPort}")
  ...
  def system: ActorSystem = ActorSystem(ClusterSettings.clusterName, config)

  def sharedStorePath =
    s"akka.tcp://aggregate-cluster@${hostname}:${ClusterSettings.seedPort}/user/store"
}
