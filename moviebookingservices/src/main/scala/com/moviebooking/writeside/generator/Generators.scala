...
object Generators {
  ...
  def theatres =
    List(("City Pride", Address("Seasons Mall", "Pune")), ("PVR Cinemas", Address("One Mall", "Pune")))
  ...
  def movies =
    List(
      MovieState(
        ...
        "Action",
        Map("image" → "PacificRim.jpg", "video" → "https://www.youtube.com/embed/xjDjIWPwcPU")
      ),
      MovieState(
        "Black Panther",
        ...
        "Adventure",
        Map("image" → "Black Panther.jpg", "video" → "https://www.youtube.com/embed/8BAhwgjMvnM")
      ),
      MovieState(
        ...
        "Adventure",
        Map("image" → "Bahubali.jpg", "video" → "https://www.youtube.com/watch?v=3NQRhE772b0")
      )
    )

  def generateShowIds = {
    screenIds.map(id ⇒ ShowId(id, getRandom(showTimes), getRandom(theatreNames)))
  }
